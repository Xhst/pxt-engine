...
namespace PXTEngine {
	...
	void Image::createTextureImage(const char* filename) {
		...
		stbi_uc* pixels = stbi_load(filename, &width, &height, &bytesPerPixel, STBI_rgb_alpha);
		...
		m_device.transitionImageLayout(m_image, m_imageFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

		// then we copy the contents of the image (that were inside the stagingBuffer) into the vkImage
		m_device.copyBufferToImage(stagingBuffer->getBuffer(), m_image, static_cast<uint32_t>(width), static_cast<uint32_t>(height));
	...
	void Image::createTextureImageView() {
		m_imageView = m_device.createImageView(m_image, m_imageFormat);
	}
	...
	void Image::createTextureSampler() {
		...
		samplerInfo.magFilter = VK_FILTER_NEAREST; // nearest is pixelated, linear is smooth
		...
		samplerInfo.maxAnisotropy = 1.0; //m_device.properties.limits.maxSamplerAnisotropy;
		...
		samplerInfo.compareOp = VK_COMPARE_OP_NEVER; // https://developer.nvidia.com/gpugems/gpugems/part-ii-lighting-and-shadows/chapter-11-shadow-map-antialiasing
