...
namespace PXTEngine {
	...
	void RayTracingRenderSystem::createShaderBindingTable() {
		...
		std::vector<uint8_t> shaderHandleStorage(sbtSize); // Use sbtSize directly, as it accounts for alignment padding

		// Ensure vkGetRayTracingShaderGroupHandlesKHR function pointer is loaded
		if (vkGetRayTracingShaderGroupHandlesKHR(m_context.getDevice(), m_pipeline->getHandle(), 0, numGroups, sbtSize, shaderHandleStorage.data()) != VK_SUCCESS) {
		...
		stagingBuffer.map();
		stagingBuffer.writeToBuffer(shaderHandleStorage.data(), sbtSize);
		stagingBuffer.unmap();
